...
def get_data():
    ...
    df['content'] = df['content'].replace(r'http\S+', '', regex=True).replace(r'www\S+', '', regex=True).replace(
...
def main():
    ...
    text_seed = input("Enter your tweet topic: ")
    tweet_length = int(input("Enter the number of lines of your tweet: "))
    if text_seed is not None and tweet_length > 0:
        for i in range(10):
            text = []
