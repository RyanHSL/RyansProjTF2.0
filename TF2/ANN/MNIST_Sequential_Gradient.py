...
def train(epoch, model, optimier):
    train_ds = get_data()
    loss = accuracy = 0.0

    for step, (x, y) in enumerate(train_ds):
        loss, accuracy = train_one_step(model, optimier, x, y)
        if step % 500 == 0:
...
def main():
    ...
    model = Sequential([keras.layers.Reshape(target_shape=(28*28,), input_shape=(28, 28)), # Flatten layer
